...
function drawHeader(doc: jsPDF, day: DayLog): void {
  ...
  doc.setFontSize(5);
  doc.text("ORIGINAL — Submit to carrier within 13 days", PW - MR, 7, {
    align: "right",
  });
  doc.text(
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(6);
  doc.text("I certify that these entries are true and correct", PW * 0.6, 51, {
    align: "left",
  });
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(5);
  doc.text("(NAME OF CARRIER OR CARRIERS)", PW / 4 + 4, 65.5, {
    align: "center",
  });
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(5);
  doc.text("(DRIVER'S SIGNATURE IN FULL)", PW * 0.76, 65.5, {
    align: "center",
  });
  ...
  // Co-driver name
  const coDriver =
    day.co_driver && day.co_driver.trim() && day.co_driver !== "N/A"
      ? day.co_driver
      : "—";
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  ...
  doc.text(
    ...
    PW / 2,
    87,
    { align: "center", maxWidth: GRID_W + 20 },
  );
  doc.setTextColor(0);
}
...
function drawGridAndLabels(doc: jsPDF): void {
  ...
  for (const [key, lines] of rowLabels) {
    ...
    for (let li = 0; li < lines.length; li++) {
      doc.text(lines[li]!, GRID_LEFT - 1.5, startY + li * lineH, {
        align: "right",
      });
    }
  }
...
function drawStatusLines(doc: jsPDF, entries: LogEntry[]): void {
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    const x2 =
      i + 1 < sorted.length ? timeToX(sorted[i + 1]!.time) : GRID_RIGHT;
...
function drawRemarks(doc: jsPDF, entries: LogEntry[], rulerY: number): void {
  ...
  const sorted = [...entries]
    ...
    .filter(
      (e) => e.status === "ON_DUTY_NOT_DRIVING" && e.location.trim() !== "",
    );
  ...
  for (const entry of sorted) {
    ...
    const label =
      entry.location + (entry.remarks?.trim() ? ` ${entry.remarks}` : "");
    doc.setFont("helvetica", "normal");
    doc.setFontSize(6);
